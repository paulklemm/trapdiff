...
# Now filter genes that do not have at least one sample more than counts_min counts
# Get gene ids that satisfy the condition
genes_satisfying_min_counts <-
  counts %>%
  ...
  dplyr::filter(count >= rmd_params$counts_min) %>%
  dplyr::select(ensembl_gene_id) %>%
  dplyr::distinct() %>%
  dplyr::pull()
...
We keep `r genes_satisfying_min_counts %>% length()` genes where at least one sample has a count of `r rmd_params$counts_min` or more.
...
de <- dplyr::bind_rows(
  ...
  source_effect %>%
    select_join() %>%
    dplyr::mutate(comparison = "source_effect")
) %>%
...
FC Treatment in IP and Input with color of IP vs Input.

```{r additional-de-plot}
de %>%
  tidyr::pivot_wider(names_from = comparison, values_from = c(padj, log2FoldChange)) %>%
  dplyr::filter(padj_source_effect <= 0.05) %>%
  ggplot2::ggplot(
    mapping = ggplot2::aes(
      x = log2FoldChange_main_effect_ip,
      y = log2FoldChange_main_effect_input,
      color = log2FoldChange_source_effect
    )
  ) +
  ggplot2::geom_point(
    alpha = 0.3
  ...
  ggplot2::scale_color_distiller(palette = "RdBu", type = "div", direction = -1, limits = c(-5, 5)) +
  ggplot2::theme_minimal() +
  ggplot2::ggtitle("FC Treatment in IP and Input with color of IP vs Input")
```
...
de %>%
  ...
  dplyr::mutate(
    source_regulated = padj_source_effect <= 0.05,
    interaction_regulated = padj_interaction_effect <= 0.05
  ) %>%
  dplyr::select(source_regulated, interaction_regulated) %>%
