...
de <- dplyr::bind_rows(
  ...
  source_effect_a %>%
    select_join() %>%
    dplyr::mutate(comparison = "source_effect_a"),
  source_effect_b %>%
    select_join() %>%
    dplyr::mutate(comparison = "source_effect_b")
) %>%
...
de %>%
  ...
  ggplot2::ggplot(
    mapping = ggplot2::aes(
      x = log2FoldChange_main_effect_ip,
      y = log2FoldChange_main_effect_input,
      color = log2FoldChange_source_effect_a
    )
  ) %>%
  fc_scatterplot(
...
de %>%
  tidyr::pivot_wider(names_from = comparison, values_from = c(padj, log2FoldChange)) %>%
  dplyr::filter(padj_source_effect_b <= 0.05) %>%
  ggplot2::ggplot(
    mapping = ggplot2::aes(
      x = log2FoldChange_main_effect_ip,
      y = log2FoldChange_main_effect_input,
      color = log2FoldChange_source_effect_b
    )
  ) %>%
  fc_scatterplot(
