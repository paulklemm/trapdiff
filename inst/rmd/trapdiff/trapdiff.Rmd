...
**trapdiff** allows interaction term analysis of complex experimental setups, such as riboTRAP or bacTRAP.
...
3. Check the TPMs for the DE genes of the interaction term eiter in **DE Interaction Gene Plots** or in the interactive shiny app
...
tpms <-
  ...
  dplyr::mutate(rpk_scaling_factor = sum(reads_per_kilobase) / 1000000) %>%
  dplyr::ungroup() %>%
...
# Here we need to update the counts based on the minimum TPM filter
counts <- counts %>%
...
# We need to make sure to have sizeFactor vector in correct order
size_factors <-
  col_data %>%
  # Left join with col_data to have to correct order
  dplyr::left_join(
    dplyr::bind_rows(
      # Create tibbles from named vectors
      tibble::enframe(treatment_a_sizeFactor, name = "id", value = "sizeFactor"),
      tibble::enframe(treatment_b_sizeFactor, name = "id", value = "sizeFactor")
    ),
    by = "id"
  ) %>%
...
DESeq2::DESeqDataSetFromMatrix(
  ...
  tibble::as_tibble() %>%
  dplyr::select(row, padj, log2FoldChange) %>%
  ...
  plot_volcano(
    ...
    min_log2fc = 0,
    plot_top_n = 10,
    title = glue::glue("DESeq2 Model of {col_data$source %>% levels() %>% .[1]} vs {col_data$source %>% levels() %>% .[2]} in treatment a ({col_data$treatment %>% levels() %>% .[1]})")
  )
...
DESeq2::DESeqDataSetFromMatrix(
  ...
  plot_volcano(
    ...
    min_log2fc = 0,
    plot_top_n = 10,
    title = glue::glue("DESeq2 Model of {col_data$source %>% levels() %>% .[1]} vs {col_data$source %>% levels() %>% .[2]} in treatment a ({col_data$treatment %>% levels() %>% .[2]})")
  )
...
if (rmd_params$deseq_treatments_separately) {
...
t_test_result <- cpms_normalized$ensembl_gene_id %>%
  unique() %>%
  furrr::future_map(function(gene_id) {
  #purrr::walk(function(gene_id) {
    # print(gene_id)
    # Check if we get enough samples
    cpms_normalized_of_gene <-
      cpms_normalized %>%
      dplyr::filter(ensembl_gene_id == gene_id) %>%
      dplyr::filter(!is.infinite(cpm_norm))
    min_valid_samples <- cpms_normalized_of_gene %>%
    ...
    # Get Fold-change
    l2fc <- cpms_normalized_of_gene %>%
      dplyr::group_by(get(comparison_variable)) %>%
      dplyr::summarise(mean_cpm_norm = mean(cpm_norm), .groups = "drop_last") %>%
      tidyr::pivot_wider(names_from = `get(comparison_variable)`, values_from = mean_cpm_norm) %>%
...
dplyr::left_join(
  x = t_test_result,
  # Get wide format of normalized CPMs
  y = cpms_normalized %>%
    dplyr::mutate(sample = glue::glue("{source}_{sample_id}")) %>%
    dplyr::select(ensembl_gene_id, sample, cpm_norm) %>%
    tidyr::pivot_wider(names_from = sample, values_from = cpm_norm),
  by = "ensembl_gene_id"
) %>%
...
Click on `Code` above to display the DE genes and normalized CPM values for this comparison.
...
Plotting the fold-change comparing the IPs vs fold-change of normalized CPMs.
...
foldchange_comparison %>%
  ...
  ggplot2::ggplot(
    ...
    ggplot2::ylab(expression(log[2](fc ~ normalized ~ CPMs ~ ttest))) +
    ggplot2::labs(colour = "Significance") +
    ggplot2::theme_minimal() +
    ggplot2::ggtitle(glue::glue("IP-only comparison fold-change vs normalized CPM foldchanges ({comp_a_variable} vs {comp_b_variable})"))
...
This is only suitable for briefly checking the results.
For a proper analysis tool, check the results using the shiny application.
