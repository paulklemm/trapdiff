...
rmyknife::print_params(print_params, omit = c("counts", "tpms"))
```
...
# Also remove these samples from the tpm count matrix
# This will not affect trapdiff in any way, it just causes
# no confusion when we export the TPMs with trapdiff as well
tpms <- rmd_params$tpms
undefined_samples$id %>%
  purrr::walk(function(na_sample) {
    tpms <<- tpms %>%
      dplyr::select(-dplyr::contains(na_sample))
...
tpms <-
  ...
  dplyr::mutate(gene_length_kb = end_position - start_position) %>%
