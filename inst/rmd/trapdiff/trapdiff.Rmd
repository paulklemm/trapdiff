...
# Only if we want to do the normalization
do_normalization <- settings$normalization != "none"
...
if (do_normalization) {
  ...
  counts_normalized <-
    ...
    # Add constant 1 to avoid division by 0
    ...
    dplyr::mutate(log2_count_norm = log2(count_norm))
  ...
  ratio_t_test <-
    ...
    furrr::future_map(function(gene_id) {
      ...
      l2fc <-
        ...
        .$fc
      # Avoid running tests where all values are 0
      if (var(counts_normalized_of_gene$log2_count_norm) == 0) {
        return()
      }
      # Perform t-test
      counts_normalized_of_gene %>%
      # counts_normalized_added %>%
      #   dplyr::filter(ensembl_gene_id == gene_id) %>%
        infer::t_test(
          response = log2_count_norm,
          explanatory = source,
...
#### IP/Input normalized analysis

Here we calculate IP/Input ratios for each sample per gene and run a t-test between our experimental conditions.
This is useful for riboTRAP data. For bacTRAP data, you don't want to consider this result.

```{r t-test-volcano, warning = FALSE, mesage = FALSE}
if (do_normalization) {
  ratio_t_test %>%
    dplyr::rename(padj = p_value) %>%
    plot_volcano(
...
This is done on raw values, not IP/Input ratios.
...
```{r interactive-scatterplot-define}
...
```{r interactive-scatterplot, eval=FALSE}
