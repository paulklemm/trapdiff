...
Let's check the DE genes for each 
...
plot <- de %>%
  ...
  # crosstalk::SharedData$new(~ensembl_gene_id, group = "Choose a gene") %>%
  ggplot2::ggplot(
    mapping = ggplot2::aes(
      x = log2FoldChange_main_effect_ip,
      y = log2FoldChange_main_effect_input,
...
plot %>%
  ...
  plotly::layout(
    xaxis = list(
      rangeselector = list(type = "gene_id")
    )
  )
```
...
```{r violin-plot}
# Create tidy long format of counts
...
# Make wide format again to have unique gene_id for each row
#   dplyr::select(-group) %>%
...
# DEBUG Filter
plot_dat <- plot_dat %>%
  dplyr::filter(gene_id %in% sig_genes_id)
...
# plot_dat_sd <- crosstalk::SharedData$new(plot_dat, ~gene_id, group = "Choose a gene")
plot_dat_sd <- crosstalk::SharedData$new(
  # data = plot_dat %>% dplyr::filter(group %in% c("input_hfd", "ip_hfd", "ip_ncd")) %>% dplyr::select(-group) %>% tidyr::pivot_wider(names_from = id, values_from = value),
  # data = plot_dat %>% dplyr::filter(group %in% c("input_hfd", "ip_hfd", "ip_ncd")) %>% dplyr::filter(!is.na(external_gene_name)),
  data = plot_dat,
  # data = plot_dat %>% dplyr::filter(group %in% c("ip_ncd")) %>% dplyr::filter(!is.na(external_gene_name)),
  # data = plot_dat %>% dplyr::select(-group) %>% tidyr::pivot_wider(names_from = id, values_from = value),
  key = ~gene_id,
  group = "Choose gene"
)
...
v_plot %>%
  plotly::ggplotly() #%>%
  #plotly::highlight(on = "plotly_click", off = "plotly_deselect")

# https://github.com/rstudio/crosstalk/pull/70
# https://github.com/rstudio/crosstalk/issues/16
# widgets <- crosstalk::bscols(
#   widths = c(12),
#   crosstalk::filter_select("gene_id", "Gene ID", plot_dat_sd, ~gene_id)
# )

# crosstalk::bscols(
#   widths = c(4, 8), widgets,
#   plotly::ggplotly(violin_plot(plot_dat_sd))
# )
...
#   #   dynamic = TRUE,
#   #   selectize = TRUE,
#   #   defaultValues = "ENSMUSG00000000003_Pbsn"
#   # )
```
